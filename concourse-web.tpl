...
if [ `gsutil ls gs://${keys_bucket}/keys/web | grep tsa_host_key.pub -c` -eq 0 ]; then
...
if [ `gsutil ls gs://${keys_bucket}/ssl_cert | grep ssl_cert/concourse-web.crt -c` -eq 0 ]; then
    mkdir -p ssl_cert
...
EOF
    ...
    gsutil cp -r /home/ubuntu/ssl_cert gs://${keys_bucket}/

else
    mkdir /home/ubuntu/ssl_cert
    gsutil cp -r gs://${keys_bucket}/ssl_cert/* /home/ubuntu/ssl_cert
fi
